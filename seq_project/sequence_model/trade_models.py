...
# from tensorflow.keras.layers import Dense, Input, Dropout, LSTM, Activation, BatchNormalization
...
def trade_dec_model(input_shape, Y_num_column):
    ...
    X = LSTM(input_shape[0]*2, return_sequences=True, recurrent_dropout=0.2)(batch_layer)
    # Add dropout with a probability of 0.5
    # Propagate X trough another LSTM layer with 128-dimensional hidden state
    # Be careful, the returned output should be a single hidden state, not a batch of sequences.
    X = LSTM(input_shape[0]*2,return_sequences=False, recurrent_dropout=0.2)(X)
    # Add dropout with a probability of 0.5
